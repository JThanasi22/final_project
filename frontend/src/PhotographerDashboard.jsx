...
export default function PhotographerDashboard() {
    ...
    useEffect(() => {
        const checkDue = () => {
            const now    = Date.now();
            const last   = +localStorage.getItem('lastDueAlertPhotog') || 0;
            const window = 12 * 60 * 60 * 1000;
            if (now - last < window) return;  // already ran in this 12h block
            ...
            if (due.length) {
                ...
                localStorage.setItem('lastDueAlertPhotog', String(now));
            }
        };
